...
Devvit.addCustomPostType({
  ...
  render: (context) => {
    // Return a webview that will render our React app
    return {
      aspectRatio: 1,
      url: `https://${context.hostname}/game.html?postId=${context.postId}`,
    };
  },
});
      ...
      switch (type) {
        case 'GET_GAME_STATE': {
          ...
          if (!parsedState) {
            ...
            // Check if phase needs to be rotated
            const now = Date.now();
...
async function rotatePhase(redis: any, postId: string, gameState: any): Promise<any> {
  ...
  if (gameState.phase === 'submission' && now >= gameState.phaseEndsAt) {
    ...
    gameState.phaseEndsAt = now + (12 * 60 * 60 * 1000); // 12 hours
